...
class DiscreteInvertedPendulum(object):
    ...
    def get_reward(self, discrete_theta):
        ...
        return 0.05 + 0.95 * (self.theta_threshold_radians -
                              np.abs(self.from_discrete(discrete_theta, self.args.theta_discrete_steps,
                                                        range=[-math.pi / 2,
                                                               math.pi / 2]))) / self.theta_threshold_radians
    ...
    def get_discrete_values(self):
        ...
        discrete_theta_dot = self.to_discrete(self.theta_dot, self.args.theta_dot_discrete_steps,
    ...
    def step(self, action):
        ...
        self.reward = self.get_reward(
            self.to_discrete(self.theta, self.args.theta_discrete_steps, range=[-math.pi / 2, math.pi / 2]))
        self.total_reward = self.total_reward + self.reward
        return self.get_continuous_values()


class InvertedPendulumGame(object):
    def __init__(self, args, windowdims=(800, 400), cartdims=(50, 10), penddims=(6.0, 150.0), refreshfreq=1000,
                 mode=None):
        self.activate_disturbance = args.activate_disturbance
        self.save_picture = args.save_picture
        self.args = args
        self.RL_controller = mode
        self.max_timestep = args.max_timestep
        self.game_round_number = 0
        if args.q_table is not None:
            self.game_round_number = int(re.search(r"[0-9]+", args.q_table).group())
    ...
    def game_round(self):
        ...
        for i in range(self.max_timestep):
            disturbance = False
            if self.activate_disturbance == "True" and np.random.rand() > 0.8:
            ...
            if self.RL_controller is None:
                ...
                action = self.RL_controller.get_action(self.pendulum.get_discrete_values(),
                                                       disturbance,
                                                       random_controller=self.random_controller,
                                                       episode=self.game_round_number)
                for event in pygame.event.get():
                    if event.type == QUIT:
                        pygame.quit()
                        sys.exit()
        ...
        if self.game_round_number % 10 == 0 and self.save_picture == "True":
    ...
    def game(self):
        ...
        while True:
            if self.RL_controller is None:  # Manual mode engaged
                ...
                self.game_round()
                self.average_track[count] = np.around(np.mean(self.score_list), 3)
                self.end_of_round()
                count += 1
                self.pendulum.reset()
...
def get_args():
    ...
    parser.add_argument('--activate_disturbance', type=str, default=False)
    ...
    parser.add_argument('--save_q_table', type=str, default="False")
    parser.add_argument('--save_v_table', type=str, default="False")
    parser.add_argument('--visualize', type=str, default="True")
    parser.add_argument('--q_table', type=str, default=None)
    parser.add_argument('--v_table', type=str, default=None)
